...
// BusImagesController — BusMaintenanceLog has no BusOperatorId of its own, so scoping/ownership
// checks join through Bus.BusOperatorId. See OperatorBranchesController's header comment for the
...
namespace TicketPortal.Api.Controllers
{
    ...
    [ApiController]
    public class BusMaintenanceLogsController(AppDbContext db) : ControllerBase
    {
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            ...
            {
                return Ok(Array.Empty<BusMaintenanceLogResponseDto>());
            }

            var busOperatorId = await User.GetBusOperatorIdAsync(db);
            var query = db.BusMaintenanceLogs.AsQueryable();
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            ...
            var item = await db.BusMaintenanceLogs.FirstOrDefaultAsync(x => x.Id == id);
        ...
        [HttpPost]
        public async Task<IActionResult> Create(BusMaintenanceLogCreateDto dto)
        {
            if (!User.IsInRole("Admin") && !User.IsInRole("Staff") && !User.IsInRole("Operator")) return Forbid();
            ...
            // Scoped staff can only log maintenance against their OWN operator's buses.
            if (busOperatorId != null && busOwnerId != busOperatorId)
            {
                return BadRequest(new { message = "That Bus belongs to a different operator." });
            }

            var item = new BusMaintenanceLog
            {
                BusId = dto.BusId,
                MaintenanceDateUtc = dto.MaintenanceDateUtc,
                OdometerKm = dto.OdometerKm,
                Title = dto.Title,
                Description = dto.Description,
                Cost = dto.Cost,
                NextDueDateUtc = dto.NextDueDateUtc,
                PerformedBy = dto.PerformedBy,
            };

            db.BusMaintenanceLogs.Add(item);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, BusMaintenanceLogUpdateDto dto)
        {
            if (!User.IsInRole("Admin") && !User.IsInRole("Staff") && !User.IsInRole("Operator")) return Forbid();

            var item = await db.BusMaintenanceLogs.FirstOrDefaultAsync(x => x.Id == id);
            if (item == null) return NotFound(new { message = "BusMaintenanceLog not found." });
            ...
            if (!item.RowVersion.SequenceEqual(dto.RowVersion))
            {
                return Conflict(new
                {
                    message = "This BusMaintenanceLog was changed by another request. Please GET the latest data and try again."
                });
            }

            db.Entry(item).Property(x => x.RowVersion).OriginalValue = dto.RowVersion;

            // If the caller is moving this log to a different Bus, re-check "does it exist / whose
            // is it" against the NEW target, not just the old one.
            ...
            item.MaintenanceDateUtc = dto.MaintenanceDateUtc;
            item.OdometerKm = dto.OdometerKm;
            item.Title = dto.Title;
            item.Description = dto.Description;
            item.Cost = dto.Cost;
            item.NextDueDateUtc = dto.NextDueDateUtc;
            item.PerformedBy = dto.PerformedBy;
            ...
            {
                return Conflict(new { message = "This BusMaintenanceLog was already modified or deleted by another request." });
            }
            catch (DbUpdateException ex)
            {
                var error = ex.InnerException?.InnerException?.Message ?? ex.InnerException?.Message ?? ex.Message;
                return Conflict(new { message = "Could not save BusMaintenanceLog.", details = error });
            }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            ...
            {
                return Conflict(new { message = "Cannot delete this BusMaintenanceLog — it is still referenced by other records." });
            }

            return NoContent();
        }

        private static BusMaintenanceLogResponseDto ToResponseDto(BusMaintenanceLog x) => new()
        {
            Id = x.Id,
            BusId = x.BusId,
            MaintenanceDateUtc = x.MaintenanceDateUtc,
            OdometerKm = x.OdometerKm,
            Title = x.Title,
            Description = x.Description,
            Cost = x.Cost,
            NextDueDateUtc = x.NextDueDateUtc,
