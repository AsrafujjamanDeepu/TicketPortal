...
namespace TicketPortal.Api.Controllers
{
    ...
    public class ActivityLogsController(AppDbContext db) : ControllerBase
    {
        ...
        {
            var items = await db.ActivityLogs.ToListAsync();
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, ActivityLogUpdateDto dto)
        {
            var item = await db.ActivityLogs.FirstOrDefaultAsync(x => x.Id == id);
            if (item == null) return NotFound(new { message = "ActivityLog not found." });
            ...
            if (!item.RowVersion.SequenceEqual(dto.RowVersion))
            {
                return Conflict(new
                {
                    message = "This ActivityLog was changed by another request. Please GET the latest data and try again."
                });
            }
            ...
            {
                return Conflict(new { message = "This ActivityLog was already modified or deleted by another request." });
            }
            catch (DbUpdateException ex)
            {
                var error = ex.InnerException?.InnerException?.Message ?? ex.InnerException?.Message ?? ex.Message;
                return Conflict(new { message = "Could not save ActivityLog.", details = error });
            }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            ...
            {
                return Conflict(new { message = "Cannot delete this ActivityLog — it is still referenced by other records." });
            }
