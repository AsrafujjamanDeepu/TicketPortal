...
namespace TicketPortal.Api.Controllers
{
    ...
    public class PaymentWebhookEventsController(AppDbContext db) : ControllerBase
    {
        ...
        {
            var items = await db.PaymentWebhookEvents.ToListAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> Create(PaymentWebhookEventCreateDto dto)
        {
            var item = new PaymentWebhookEvent
            {
            ...
            db.PaymentWebhookEvents.Add(item);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(Guid id, PaymentWebhookEventUpdateDto dto)
        {
            var item = await db.PaymentWebhookEvents.FirstOrDefaultAsync(x => x.Id == id);
            if (item == null) return NotFound(new { message = "PaymentWebhookEvent not found." });
            ...
            if (!item.RowVersion.SequenceEqual(dto.RowVersion))
            {
                return Conflict(new
                {
                    message = "This PaymentWebhookEvent was changed by another request. Please GET the latest data and try again."
                });
            }
            ...
            item.ProviderEventId = dto.ProviderEventId;
            item.EventType = dto.EventType;
            item.ReportedStatus = dto.ReportedStatus;
            ...
            {
                return Conflict(new { message = "This PaymentWebhookEvent was already modified or deleted by another request." });
            }
            catch (DbUpdateException ex)
            {
                var error = ex.InnerException?.InnerException?.Message ?? ex.InnerException?.Message ?? ex.Message;
                return Conflict(new { message = "Could not save PaymentWebhookEvent.", details = error });
            }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            ...
            {
                return Conflict(new { message = "Cannot delete this PaymentWebhookEvent — it is still referenced by other records." });
            }
